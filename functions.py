...
def welcomeText(st):
    ...
    st.subheader(
        "To get started, just select *start* from the selecbox is the side bar on the left.")
...
def describeData(st, df, listOfColumnNames):
    ...
    select = st.sidebar.selectbox('Select an option', [
                                  'Sum', 'Quartile', 'Median', 'Lowest', 'Highest', 'Mean', 'Variance', 'Standard Deviation', 'Comprehensive'])
    options = []
    ...
    # Get lowest value or rrecord of highest value out of all data in column
        ...
        if select2 == 'Yes':
            size = st.number_input(
                'Enter the sample size', min_value=1, step=1, max_value=len(df))
            smallest = df[columnNames].nsmallest(size)

            select3 = st.sidebar.selectbox(
                'Minimal or Whole', ['Minimal', 'Whole'])

            if select3 == 'Minimal':
                st.write('The lowest records by ',
                         columnNames, ' are: ', smallest)
    ...
    # Get highest value or record of highest value out of all data in column
        ...
        if select2 == 'Yes':
            ...
            if select3 == 'Minimal':
                st.write('The highest records by ',
                         columnNames, ' are: ', largest)
    ...
    # Get standard deviation of all values in column
        ...
        columnNames = st.sidebar.selectbox('Column Names', options)
        displayStatistics(getStdOfColumn, columnNames,
...
def lengthOption(st, listOfColumnNames, df):
    ...
    if columnNames == 'All (length for all columns)':
        ...
        st.write('The length of column ', columnNames,
                 ' in the dataframe is: ', getLength(columnNames, df))
...
def graphOption(st, listOfColumnNames, df, indexColumn):
    ...
    select = st.sidebar.selectbox(
        'Pick a graph', ['Area Chart', 'Line Chart', 'Bar Chart', 'Scatter Plot'])
    ...
    if select == 'Area Chart':
        ...
        size = st.sidebar.number_input(
            'Sample size', step=1, min_value=1, max_value=len(df))
        ...
        if select2 == 'Yes':
            ...
            tooltips = st.sidebar.multiselect(
                'Tooltips (select more than one', listOfColumnNames)

            title = st.text_input('Chart Title')
            width = st.number_input(
                'Width', step=10, min_value=200, max_value=1000)
            height = st.number_input(
                'Height', step=10, min_value=400, max_value=1000)
            lineColor = st.sidebar.color_picker('Line color', value='#923E94')
            gradientColor = st.sidebar.color_picker(
                'Gradient color', value='#923E94')
    ...
    # Display bar chart
        ...
        if select2 == 'Yes':
            ...
            color = st.sidebar.selectbox(
                'Change color based of value?', ['No', 'Yes'])
            width = st.sidebar.selectbox(
                'Change width based of value?', ['No', 'Yes'])
            tooltips = st.sidebar.multiselect(
                'Tooltips (select more than one', listOfColumnNames)
        ...
        # Display simple bar chart
            ...
            columnNames = st.sidebar.selectbox(
                'Choose a column to display', options)
...
def sampleOption(df, st):
    sampleSelect = st.sidebar.selectbox(
        'Whole or Sample', ['Sample for dataframe', 'Whole dataframe'])
    if sampleSelect == 'Sample for dataframe':
        size = st.number_input('Enter the sample size',
                               min_value=1, step=1, max_value=len(df))

        sampleSelect2 = st.sidebar.selectbox(
            'Sample', ['Sample from top', 'Sample from bottom', 'Random sample'])
...
def getHelpDetails(st):
    option = st.sidebar.selectbox('FAQ', ['How to upload a CSV file?', 'How to start SAM?', 'Assign index to data frame?', 'Display sample or whole data frame?',
                                  'Length of column or whole data frame?', 'Visualize data and draw graphs?', 'How to statistically describe data?'])
    ...
    if option == 'How to upload a CSV file?':
        ...
        st.write(
            '5) When your file is successfully uploaded, select *Yes* from Start select box')
        st.write('')
        ...
        st.write('4) Then select *Yes* from *Start* select box')
        st.write(
            '5) The *Set Index from unique columns* select box display all in the unique column in the data frame')
        st.write(
            '6) Not every column display should be used, select whichever column would be the best for your analysis')
        st.write('')
        st.write('Need more help? Watch the video below:')
        ...
        st.write('4) Then select *Yes* from *Start* select box')
        st.write(
            '5) Select *Get Sample* from *Main Options* select box. Two select boxes will appear below')
        ...
        st.write('4) Then select *Yes* from *Start* select box')
        st.write(
            '5) Select *Get Length* from *Main Options* select box. A select box will appear below')
        ...
        st.write('4) Then select *Yes* from *Start* select box')
        st.write(
            '5) Select *Draw Graph* from *Main Options* select box. Three select boxes will appear below')
        ...
        st.write('4) Then select *Yes* from *Start* select box')
        st.write(
            '5) Select *Describe Data* from *Main Options* select box. Two select boxes will appear below')
